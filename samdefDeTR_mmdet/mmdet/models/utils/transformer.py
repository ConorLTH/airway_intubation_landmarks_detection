...
class AdaptivePadding(nn.Module):
    ...
    This AdaptivePadding class is a custom implementation of a padding layer in PyTorch.
    When initialized, it takes in several arguments such as kernel_size, stride, dilation, and padding mode.
    These arguments are used to calculate the amount of padding needed for the input tensor.

    The forward method takes in an input tensor x and calculates the amount of padding needed for 
    the height and width dimensions using the get_pad_shape method.
    The amount of padding is calculated based on the input shape, kernel size, stride, and dilation.
    If padding is needed, it is applied to the input tensor using PyTorch’s F.pad function.
    The padded tensor is then returned.

    The padding mode can be either same or corner.
    If the padding mode is corner, the padding is applied to the bottom and right sides of the input tensor.
    If the padding mode is same, the padding is evenly distributed on all sides of the input tensor.
...
@TRANSFORMER.register_module()
class DNTransformer(BaseModule):
    """Implements the DETR transformer.
    ...
    def forward(self, x, mask, refpoint_embed, pos_embed ,fc_reg, activate, reg_ffn, target, attn_mask):
        """Forward function for `Transformer`.
...
class DeformableDetrTransformerDecoder(TransformerLayerSequence):
    ...
    def forward(self,
                ...
                **kwargs):
        """Forward function for `TransformerDecoder`.
        # 在DeformableDetrTransformer 中有对reference points的初始化,在送入decoder之前
...
class DeformableDetrTransformer(Transformer):
    ...
    def forward(self,
        ...
        for lvl, (feat, mask, pos_embed) in enumerate(
            ...
            feat = feat.flatten(2).transpose(1, 2) #(bs, h*w, c) c=256
            mask = mask.flatten(1)
        ...
        #feat_flatten 的形状为 [bs, num_levels * h * w, c]，其中 c 是特征的通道数。
        #mask_flatten 的形状为 [bs, num_levels * h * w]。
        #lvl_pos_embed_flatten 的形状为 [bs, num_levels * h * w, embed_dim]，其中 embed_dim 是位置嵌入的维度。
        #spatial_shapes 的形状为 [num_levels, 2]，其中每一行表示对应层的特征图的高和宽。
        #level_start_index 的形状为 [num_levels]，表示每一层特征图在扁平化后的起始索引。
        #valid_ratios 的形状为 [bs, num_levels, 2]，表示每一层特征图的有效区域占比

        reference_points = \
            self.get_reference_points(spatial_shapes,
                                      valid_ratios,
                                      device=feat.device)
        #Encoder 的reference points 通过get_reference_points获得
        #与Decoder的不同

        feat_flatten = feat_flatten.permute(1, 0, 2)  # (N, bs, embed_dims) N=sum(H_*W_ for _ in [8,16,32,64]) num_levels=4 
        ...
        memory = self.encoder(
            ...
            **kwargs)
        # memory(N, bs, embed_dims)

        memory = memory.permute(1, 0, 2) #memory(bs,N,embed_dims)
        bs, _, c = memory.shape
        if self.as_two_stage:
            ...
            query_pos, query = torch.split(query_embed, c, dim=1) #split [num_queries,hidden_dims] typically [300,256]
            query_pos = query_pos.unsqueeze(0).expand(bs, -1, -1) #add dimension for batchsize [bs,num_queries,hidden_dims]
            query = query.unsqueeze(0).expand(bs, -1, -1) #add dimension for batchsize [bs,num_queries,hidden_dims]
            reference_points = self.reference_points(query_pos).sigmoid() #[bs, num_queries, 2] sigmoid归一化，将坐标值转到0,1之间
            # self.reference_points = nn.Linear(256,2)
            # 参考点是通过一个线性层从查询位置预测得到的，并通过 sigmoid 函数将其值限制在 0 到 1 之间
            init_reference_out = reference_points

        # decoder
        query = query.permute(1, 0, 2)  # [bs,num_queries,hidden_dims] --> [num_queries,bs,hidden_dims]
        memory = memory.permute(1, 0, 2) # [bs,num_queries,hidden_dims] --> [num_queries,bs,hidden_dims]
        query_pos = query_pos.permute(1, 0, 2) # [bs,num_queries,hidden_dims] --> [num_queries,bs,hidden_dims]
...
@TRANSFORMER_LAYER.register_module()
class SAMDeformableDetrTransformerDecoderLayer(BaseTransformerLayer):
    ...
    def __init__(self,
                 ...
                 ffn_num_fcs=2,
                 **kwargs):
        super(SAMDeformableDetrTransformerDecoderLayer, self).__init__(
            attn_cfgs=attn_cfgs,
            feedforward_channels=feedforward_channels,
            ffn_dropout=ffn_dropout,
            operation_order=operation_order,
        ...
        self.outproj = nn.Linear(self.d_model * self.nheads , self.d_model)
    ...
    def forward(self,
        ...
        #print('into SAMDecoderLyerForward into SAMDecoderLyerForward into SAMDecoderLyerForward into SAMDecoderLyerForward')
        norm_index = 0
        attn_index = 0
        ffn_index = 0
        ...
        for m in range(len(mems)):
            ...
            memoryh = mems_h[m]
            memoryw = mems_w[m]
            mem2d = mem.view(memoryh,memoryw,bs,c)
            mem2d = mem2d.permute(2,3,0,1) #[bs,c,memh,memw]
            mems2d.append(mem2d)
        ...
        for layer in self.operation_order:
            if layer == 'self_attn':
                ...
                memid = lid % 4
                reference_boxes_xyxy = self.box_cxcywh_to_xyxy(reference_boxes)
                reference_boxes_xyxy[:, :, 0] *= mems_w[memid]
                reference_boxes_xyxy[:, :, 1] *= mems_h[memid]
                reference_boxes_xyxy[:, :, 2] *= mems_w[memid]
                reference_boxes_xyxy[:, :, 3] *= mems_h[memid]
                valid_ratio = valid_ratios[:,memid,:].repeat(1,2)
                reference_boxes_xyxy = reference_boxes_xyxy * valid_ratio.view(bs, 1, 4)
                memory_2d = mems2d[memid]
                ...
                q_content_index = q_content.view(bs*num_query,-1,7,7)
                points = self.point1(q_content_index)
                points = points.reshape(bs*num_query,-1)
                points = self.point2(points)
                points = points.view(bs * num_query, 1, self.nheads, 2).tanh()

                q_content = F.grid_sample(q_content, points, padding_mode="zeros", align_corners=False).view(bs * num_query, -1)
                q_content = q_content.view(bs, num_query, -1, 8).permute(1, 0, 3, 2)   # (num_query, bs, n_head, 256)
                q_content = q_content * self.attn1(query).view(num_query, bs, self.nheads, self.d_model).sigmoid()
                # attn1 nn.Linear(256,256*8) 将identity从256-->256*8,又view变形成(:,:,8,256)
                # q_content attn1 投一次

                q_pos_center = reference_boxes[:, :, :2].reshape(bs, num_query, 1, 2).expand(-1, -1, self.nheads, -1) #reference_boxes:cx,cy,w,h
                q_pos_scale = reference_boxes[:, :, 2:].reshape(bs, num_query, 1, 2).expand(-1, -1, self.nheads, -1) * 0.5 #keep in the reference boxes so multiply by 0.5
                q_pos_delta = points.reshape(bs, num_query, self.nheads, 2)
                q_pos = q_pos_center + q_pos_scale * q_pos_delta
                q_pos = q_pos.permute(1, 0, 2, 3)   # (num_query, bs, n_head, 2)
                q_pos = q_pos.reshape(num_query, bs * self.nheads, 2)
                q_pos = self.gen_sineembed_for_position(q_pos).reshape(num_query, bs, self.nheads, c)
                q_pos = q_pos * self.attn2(query).view(num_query, bs, self.nheads, self.d_model).sigmoid()

                # q_pos attn2投一次
                # 分成attn1 & attn2 应该是为了梯度下降更新的时候，分开更新

                query = q_content.reshape(num_query, bs, self.nheads * c) # (num_query, bs, n_head, 256)
                query_pos = q_pos.reshape(num_query, bs, self.nheads * c)
                v = value.view(-1, bs, 1, 256).expand(-1,-1,self.nheads,-1)
                value = v.reshape(-1,bs,self.nheads * c)
...
@TRANSFORMER_LAYER_SEQUENCE.register_module()
class SAMDeformableDetrTransformerDecoder(TransformerLayerSequence):
    """Implements the decoder in DETR transformer.
    ...
    def __init__(self, *args, return_intermediate=False, **kwargs):
        ...
        super(SAMDeformableDetrTransformerDecoder, self).__init__(*args, **kwargs)
        self.return_intermediate = return_intermediate
        self.bbox_embed = None
        self.reference_points = nn.Linear(4,2)
    ...
    def forward(self,
                query,
                *args,
                reference=None, #reference_boxes [nq,bs,4]
                valid_ratios=None,
                reg_branches=None,
                **kwargs):
        #print('into SAM DecoderForward into SAM DecoderForward into SAM DecoderForward into SAM DecoderForward')
        output = query
        intermediate = []
        intermediate_reference_points = []
        ...
        for lid, layer in enumerate(self.layers):
            if lid == 0:
                reference_boxes_before_sigmoid = reference #[nq,bs,4]
                reference_boxes = reference_boxes_before_sigmoid.sigmoid().transpose(0,1) #[bs,nq,4]
            else:
                ...
                reference_boxes = reference_boxes.detach() #[bs,nq,4]
            # change obj_center = reference_boxes[...,:2] #[bs,nq,2]
            # change reference_points=obj_center #[bs,nq,2]
            reference_points = self.reference_points(reference_boxes) #[bs,nq,2]
            ...
            # reg_branches=None this part is useless
            if reg_branches is not None:
                tmp = reg_branches[lid](output)
                if reference_points.shape[-1] == 4:
                    new_reference_points = tmp + inverse_sigmoid(
        ...
        return output, reference_boxes

@TRANSFORMER.register_module()
class SAMDeformableDetrTransformer(Transformer):
    """Implements the DeformableDETR transformer.
    ...
    def __init__(self,
                 ...
                 two_stage_num_proposals=300,
                 **kwargs):
        super(SAMDeformableDetrTransformer, self).__init__(**kwargs)
        self.as_two_stage = as_two_stage
        self.num_feature_levels = num_feature_levels
        self.two_stage_num_proposals = two_stage_num_proposals
        self.embed_dims = self.encoder.embed_dims
    ...
    def init_layers(self):
        ...
        self.level_embeds = nn.Parameter(
            torch.Tensor(self.num_feature_levels, self.embed_dims))
        # change self.reference_points = nn.Linear(self.embed_dims, 2)
        self.reference_boxes = nn.Linear(self.embed_dims,4)
    ...
    def init_weights(self):
        ...
        if not self.as_two_stage:
            xavier_init(self.decoder.reference_points, distribution='uniform', bias=0.)
            xavier_init(self.reference_boxes, distribution='uniform', bias=0.)
    ...
    def forward(self,
        ...
        #print('into SAM Forward into SAM Forward into SAM Forward into SAM Forward')
        feat_flatten = []
        mask_flatten = []
        lvl_pos_embed_flatten = []
        spatial_shapes = []
        ...
        memory = memory.permute(1, 0, 2) #[bs,N,256]
        bs, _, c = memory.shape
        memory_flatten=[]
        mem0 = memory[:, level_start_index[0]: level_start_index[1], :]
        mem1 = memory[:, level_start_index[1]: level_start_index[2], :]
        mem2 = memory[:, level_start_index[2]: level_start_index[3], :]
        mem3 = memory[:, level_start_index[3]: , :]
        for m in [mem0,mem1, mem2, mem3]:
            memory_flatten.append(m.permute(1, 0, 2)) #[hw,bs,c]

        query_pos, query = torch.split(query_embed, c, dim=1)
        query_pos = query_pos.unsqueeze(0).expand(bs, -1, -1) #[bs,nq,256]
        query = query.unsqueeze(0).expand(bs, -1, -1)

        # decoder
        query = query.permute(1, 0, 2) #[nq,bs,256]
        memory = memory.permute(1, 0, 2) #[N,bs,256]
        query_pos = query_pos.permute(1, 0, 2) #[nq,bs,256]
        reference_boxes = self.reference_boxes(query_pos) #[nq,bs,4]
        init_reference_out = reference_boxes.permute(1,0,2) #[bs,nq,4]
